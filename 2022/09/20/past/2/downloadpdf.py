...
path = "D:\\userdata\\download\\Pb\\"   #下载后的PDF存放的文件夹
if os.path.exists(path) == False:
	os.mkdir(path)  #20210607更新，创建保存下载文章的文件夹
f = open("D:\\userdata\\download\\Pb.txt", "r", encoding="utf-8-sig")  #存放DOI码的.txt文件中，每行存放一个文献的DOI码，完毕须换行（最后一个也须换行！）
head = {\
...
for line in f.readlines():
	...
	try:
		...
		with open("Pberror.txt", "a+") as error:
			error.write(line + "\n")
